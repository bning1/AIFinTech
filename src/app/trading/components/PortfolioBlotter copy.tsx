...
export default function PortfolioBlotter({ desk }: { desk?: string }) {
  ...
  useEffect(() => {
    const url = '/api/trades/bond' + (desk ? '?desk=' + desk : '');
    fetch(url)
      .then(res => res.json())
      .then(data => {
  ...
  const deleteTrade = (tradeId) => {
    ...
    fetch('/api/trades/bond/' + tradeId, { method: 'DELETE' })
      .then(res => res.ok ? location.reload() : alert('Delete failed'))
      .catch(err => alert(err));
  };

  const updateQuantity = (tradeId, qty) => {
    fetch('/api/trades/bond/' + tradeId, {
      method: 'PATCH',
...
return (
    <div className="glass p-6 h-full overflow-auto">
      ...
      {Object.entries(holdings).length > 0 && (
        <>
          <h4 className="text-slate-900 mt-4 mb-2">Net Holdings</h4>
        ...
        <tbody>
          {trades.map(t => (
            <tr key={t.trade_id} className="border-b border-white/10">
              ...
              <td className="p-2">
                <button onClick={() => deleteTrade(t.trade_id)} className="text-red-500 bg-transparent border-none cursor-pointer">
                  ❌
                </button>
              </td>
            </tr>
